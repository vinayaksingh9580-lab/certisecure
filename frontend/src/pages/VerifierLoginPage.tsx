...
export default function VerifierLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tokenResponse.user.role !== 'verifier') {
        logout();
        setError('Unauthorized: Not a verifier account.');
        setLoading(false);
        return;
      }
      localStorage.setItem('user_type', 'verifier');
      navigate('/dashboard'); // Verifiers go to dashboard
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      {/* Glow effects */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[500px] h-[300px] bg-emerald-600/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Verifier Sign In</h1>
          <p className="text-gray-400 text-sm mt-1">Access the Employer / Verifier Portal</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl border border-gray-800 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold text-sm shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 transition-all disabled:opacity-50"
            >
              {loading ? (
